...
### 第三块：`tools/test_gate.py`

闸门天天全绿，**不代表它报错时报得对** —— 失败路径在绿跑时一行都不执行。
Codex 已经连着在 `verify_courseware.py` 里找出三个缺陷，**三个都在失败路径上**：
白名单按题号整体豁免、渲染失败仍自称通过、大纲缺周时 KeyError 崩溃。

```bash
python3 tools/test_gate.py     # 10 项，喂错误输入，检查闸门报错报得对不对
```

三个已知缺陷都有对应回归用例；**回退任一修复，对应用例立刻变红**（已实测 3/3）。
另含一条反向对照（正常输入下必须零失败），防止用例写成恒真。
